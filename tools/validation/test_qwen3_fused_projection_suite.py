...
class CommandScopeTests(unittest.TestCase):
    ...
    def test_benchmarks_use_real_http_cell_runner(self):
        with tempfile.TemporaryDirectory() as directory:
            args = types.SimpleNamespace(
                output_dir=directory,
                model_path="models/Qwen3-4B",
            ...
            with contextlib.redirect_stdout(io.StringIO()):
                self.assertTrue(suite.benchmarks())

            commands = [entry["command"] for entry in suite.manifest["commands"]]
            self.assertTrue(all("benchmark-cell" in command for command in commands))
            self.assertTrue(all("bench_serving" not in command for command in commands))
                                ...
                                SUITE.atomic_json(
                                    raw / name,
                                    {
                                        "qwen3_projection_fusion": {
                                            ...
                                            "qkv": qkv,
                                            "gate_up": gate_up,
                                            "resolved": SUITE.expected_fusion_plan(qkv, gate_up),
                                        },
                                        "workload": {
                                            "prompt_words": profile["prompt_words"],
