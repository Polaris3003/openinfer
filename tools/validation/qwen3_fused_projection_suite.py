...
PROFILES = {
    "prefill": {"prompt_words": 10_000, "output_len": 1, "metric": ("ttft", "p50_ms")},
    "decode": {
        "prompt_words": 1_024,
        "output_len": 256,
        "metric": ("tpot", "p50_ms"),
...
def fusion_decision(mode: str) -> dict[str, Any]:
    return {
        "fused": mode == "fused",
        "reason": "forced_fused" if mode == "fused" else "explicit_split",
    }


def expected_fusion_plan(qkv: str, gate_up: str) -> dict[str, Any]:
    return {
        "decode_qkv": fusion_decision(qkv),
        "decode_gate_up": fusion_decision(gate_up),
        "prefill_unified_qkv": fusion_decision(qkv),
        "prefill_unified_gate_up": fusion_decision(gate_up),
    }
...
def run_http_benchmark_cell(args: argparse.Namespace) -> int:
    ...
    server_log_path = output.with_suffix(".server.log")
    base_url = f"http://127.0.0.1:{args.port}"
    served_model = "qwen3-fused-projection-parity"
    server_command = [
        "cargo",
        "run",
        "--release",
    ...
    client_command = [
        sys.executable,
        str(ROOT / "scripts/bench_http_serving.py"),
        "--base-url",
        base_url,
        "--model",
        served_model,
        ...
        "--num-requests",
        str(args.iters * args.concurrency),
        "--concurrency",
    ...
    with server_log_path.open("w") as server_log:
        ...
        try:
            ...
            report["qwen3_projection_fusion"] = {
                "tp_size": args.tp_size,
                "qkv": args.qkv_fusion,
                "gate_up": args.gate_up_fusion,
                "resolved": expected_fusion_plan(args.qkv_fusion, args.gate_up_fusion),
    ...
    def initialize(self) -> None:
        ...
        self.manifest["environment"] = {
            ...
            "cuda_env": {
                ...
                if key.startswith(("CUDA", "NCCL", "PEGAINFER_CUDA"))
            },
            "model": (
    ...
    def correctness(self) -> bool:
        commands = [
            ...
            (
                ...
                {"kind": "correctness", "gate": "qwen3-server-cli"},
            ),
            (
                "operator-qkv-split",
                [
                    ...
                    for mode in MODE_ORDER:
                        ...
                        ok = self.run(
                            ...
                            [
                                ...
                                str(output),
                                "--port",
                                str(self.args.benchmark_port),
                                "--prompt-words",
                                str(profile["prompt_words"]),
                                "--output-len",
...
def benchmark_index(
    ...
    for entry in commands:
        ...
        try:
            report = load_command_output(output_dir, entry)
            qkv, gate_up = MODES[key[3]]
            fusion = report.get("qwen3_projection_fusion", {})
            if fusion.get("tp_size") != key[0]:
                raise ValueError("report TP metadata mismatch")
            if (fusion.get("qkv"), fusion.get("gate_up")) != (qkv, gate_up):
                raise ValueError(
                    f"report fusion metadata mismatch: {fusion} != {(qkv, gate_up)}"
                )
            expected_resolved = expected_fusion_plan(qkv, gate_up)
            if fusion.get("resolved") != expected_resolved:
                raise ValueError(
                    "report resolved fusion metadata mismatch: "
                    f"{fusion.get('resolved')} "
                    f"!= {expected_resolved}"
                )
            profile = PROFILES[key[1]]
            if report["workload"]["prompt_words"] != profile["prompt_words"]:
                raise ValueError("report prompt-word workload mismatch")
            if report["workload"]["max_tokens"] != profile["output_len"]:
...
def summarize(output_dir: pathlib.Path) -> dict[str, Any]:
    ...
    if manifest.get("schema") != SCHEMA:
        raise ValueError(
            f"validation artifact schema {manifest.get('schema')} != required {SCHEMA}; "
            "rerun the suite on the current real-HTTP benchmark contract"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--benchmark-port", type=int, default=18080)
    ...
    cell = subparsers.add_parser(
        "benchmark-cell", help="启动真实 HTTP server 并执行一个 matched A/B cell"
    )
    cell.add_argument("--model-path", required=True)
    cell.add_argument("--tp-size", type=int, required=True)
    cell.add_argument("--qkv-fusion", choices=("split", "fused"), required=True)
    cell.add_argument("--gate-up-fusion", choices=("split", "fused"), required=True)
    cell.add_argument("--out", required=True)
    cell.add_argument("--port", type=int, default=18080)
    cell.add_argument("--prompt-words", type=int, required=True)
    cell.add_argument("--output-len", type=int, required=True)
    cell.add_argument("--concurrency", type=int, required=True)
    cell.add_argument("--warmup", type=int, required=True)
    cell.add_argument("--iters", type=int, required=True)
    cell.add_argument("--seed", type=int, required=True)
    cell.add_argument("--ready-timeout", type=float, default=180.0)
    cell.add_argument("--request-timeout", type=float, default=600.0)
    return parser
...
def normalize_run_args(args: argparse.Namespace) -> None:
    ...
    for name in ("ready_timeout", "request_timeout"):
        if getattr(args, name) <= 0:
            raise SystemExit(f"--{name.replace('_', '-')} 必须 > 0")
