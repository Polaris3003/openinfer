...
use pegainfer_kernels::ops::GEMM_LT_MAX_N;
...
impl From<pegainfer_kernels::ops::GemmLtAlgorithmMetadata> for AlgorithmMetadata {
    fn from(value: pegainfer_kernels::ops::GemmLtAlgorithmMetadata) -> Self {
        Self {
...
fn shape_algorithms(model: &Qwen3Model, tokens: usize) -> Result<ShapeAlgorithms> {
    ...
    let query = |rows| -> Result<Option<AlgorithmMetadata>> {
        Ok(
            pegainfer_kernels::ops::gemm_lt_algorithm_metadata(rows, tokens, hidden)?
                .map(Into::into),
        )
    };
...
fn report_layer(
    ...
    let qkv_fused_timing = measure(ctx, warmup, iters, || {
        ...
        pegainfer_kernels::ops::split_qkv_into(ctx, &qkv, &mut fused_q, &mut fused_k, &mut fused_v)
    })?;
...
pub fn generate_projection_report(options: &ProjectionReportOptions) -> Result<ProjectionReport> {
    ...
    pegainfer_kernels::ops::set_numeric_policy(NumericPolicy::Tuned);
