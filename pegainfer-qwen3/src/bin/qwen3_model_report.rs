...
use half::bf16;
use pegainfer_bench::Accum;
use pegainfer_bench::CallSiteRow;
use pegainfer_bench::LatencyStats;
use pegainfer_bench::RollupRow;
use pegainfer_bench::accumulate;
use pegainfer_bench::attr_usize;
use pegainfer_bench::axis;
use pegainfer_bench::call_site_row;
use pegainfer_bench::input;
use pegainfer_bench::output;
use pegainfer_bench::rollup_row;
use pegainfer_bench::zero_matrix;
...
use pegainfer_kernels::tensor::TensorSpec;
use pegainfer_qwen3::ProjectionFusionControl;
use pegainfer_qwen3::Qwen3ProjectionFusionOptions;
use pegainfer_qwen3::batch_decode_trace::HEAD_DIM_VALUE;
use pegainfer_qwen3::batch_decode_trace::KV_DIM_VALUE;
use pegainfer_qwen3::batch_decode_trace::MODEL;
use pegainfer_qwen3::batch_decode_trace::NUM_KV_HEADS;
use pegainfer_qwen3::batch_decode_trace::NUM_LAYERS;
use pegainfer_qwen3::batch_decode_trace::NUM_Q_HEADS;
use pegainfer_qwen3::batch_decode_trace::PHASE_DECODE;
use pegainfer_qwen3::batch_decode_trace::RMS_NORM_EPS;
use pegainfer_qwen3::batch_decode_trace::normalize_call_site;
use pegainfer_qwen3::batch_decode_trace::trace_decode_kernel_calls_with_projection_fusion;
use pegainfer_qwen3::batch_decode_trace::trace_decode_topology_calls_with_projection_fusion;
...
struct Cli {
    ...
    /// Reuse one physical KV page table across synthetic rows. Topology-only:
    /// preserves batch/sequence tensor shapes but does not model KV data locality.
    #[arg(long)]
...
fn main() -> Result<()> {
    ...
    let projection_fusion = Qwen3ProjectionFusionOptions {
        qkv: cli.qkv_fusion.resolve(),
        gate_up: cli.gate_up_fusion.resolve(),
    };
    let schedule = if cli.shared_kv_pages {
        trace_decode_topology_calls_with_projection_fusion(
            &cli.model_path,
            cli.batch_size,
            cli.kv_len,
            projection_fusion,
    ...
    let out = cli.out.unwrap_or_else(|| {
        ...
        let kv_mode = if cli.shared_kv_pages {
            "shared-kv"
        } else {
            "independent-kv"
        };
        PathBuf::from(format!(
            "target/model_reports/{MODEL}/decode-{}-qkv-{}-gate-up-{}-{kv_mode}-bs{}-kv{}.json",
            policy_name(policy),
            cli.qkv_fusion.label(),
            cli.gate_up_fusion.label(),
            cli.batch_size,
...
fn measure_paged_decode_attention(call: &KernelCall, iters: u64) -> Result<LatencyStats> {
    ...
    let (page_indices, page_indptr) = page_tables(batch, pages_per_request, total_pages)?;
...
fn page_tables(
    ...
    anyhow::ensure!(
        total_pages >= pages_per_request,
        "KV buffer has {total_pages} pages but one request needs {pages_per_request}"
    );
    let physical_requests = total_pages / pages_per_request;
    ...
    for request_idx in 0..batch {
        let physical_request = request_idx % physical_requests;
        for page_offset in 0..pages_per_request {
            page_indices.push((physical_request * pages_per_request + page_offset) as i32);
        }
        page_indptr.push(page_indices.len() as i32);
    }
    Ok((page_indices, page_indptr))
}
...
fn print_text_report(report: &ModelReport, out: &Path, dot_out: &Path) {
    ...
    println!(
        "config: bs={} kv_len={} layers={} tp={} iters={} policy={} qkv={} gate_up={} shared_kv_pages={}",
...
mod tests {
    ...
    #[test]
    fn shared_page_tables_stay_within_the_physical_buffer() {
        let (indices, indptr) = page_tables(64, 2, 3).expect("shared page table");
        assert_eq!(indices.len(), 128);
        assert_eq!(indptr.len(), 65);
        assert!(indices.iter().all(|&page| page == 0 || page == 1));
