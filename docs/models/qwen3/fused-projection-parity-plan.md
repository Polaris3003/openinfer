...
- **Risks / open questions**:
  ...
  - Issue 中“`weights.rs` 使用 `DeviceMatrix::vstack`”已过时；当前生产 loader 是 `pegainfer-qwen3/src/weights/load.rs::StagedWeightLoader::fused_rows`。

### Continuation preparation（2026-08-12）

- **Read**:
  - Issue #746 及全部评论 — 2026-08-03 已在双卡 SM86/CUDA 12.6 上完成 correctness `20/20`、projection `3/3`、topology `16/16`、E2E `64/64`；2026-08-10 维护者要求开 PR。
  - `docs/models/qwen3/fused-projection-parity-implementation-report.md` — 当前实现把 QKV、gate/up 和 decode、prefill/unified 分开解析，但报告仍停留在 GPU 实跑前状态。
  - `docs/models/qwen3/accuracy-gate.md` 与 `docs/subsystems/correctness/logits-golden-gate.md` — fused 与 split 不要求 bit-identical，但必须保持既有 regret/mean/p99 门禁，不能用 free-running exact text 或放宽阈值替代。
  - `docs/conventions/bench-regression.md` 与 `docs/playbooks/model-optimization-pipeline.md` — decode 2%、prefill 3% 是调查/资格线；每个 projection 和 phase 必须独立归因。
  - 当前 `main...HEAD` diff、`projection_fusion.rs`、decode/prefill/unified buffer 与 forward、LoRA gate/fixture generator、validation summarizer — 核对实测结论能否转化为可审查、可复现的生产策略。
- **Relevant history**:
  - SM86 数据按 suite 规则只会给 `TP1 + decode + QKV` 生成 `ENABLE`；其余七个组合均为 `KEEP_SPLIT`。
  - TP2 数据存在明显不稳定，例如 QKV decode throughput 两次复测出现 `+85.02%` 与 `-7.03%`，不能用均值掩盖方向不一致。
  - issue 评论只有汇总表；本地分支没有 raw/log/manifest/summary/decision-table/report 产物，无法独立复核环境、命令和逐 cell 结果。
  - 已提交的 `test_data/qwen3-4b-lora-golden.safetensors` 与 `main` bit-identical，仍是旧 q/v-only fixture；当前测试代码要求 q/k/v/gate/up 五 target，因此 PR 还没有自包含的 LoRA 门禁。
  - 当前 `ProjectionFusionEnvironment` 只有 numeric policy 与 decode overlap，没有 GPU SM、CUDA/cuBLAS 版本或 selected-algo 身份；直接填充 TP1 decode QKV 白名单会在所有支持的 GPU/工具链上启用，超出 SM86 证据范围。
- **Plan**:
  1. 收齐并校验 SM86 suite 原始目录与重新生成的五投影 LoRA fixture；用现有 summarizer 重建 decision table，确保 issue 汇总可从原始数据复现。
  2. 只修复与数据安全和可审查性直接相关的缺口：让 benchmark 记录完整 resolved plan，明确 Auto fallback/qualification reason，并评估 scratch enum 是否值得在本 PR 收敛；不先改 `Auto` 白名单。
  3. 运行本机可执行的格式、Python 单测、metadata/diff 门禁；在可用 Linux CUDA TP2 环境重跑受改动影响的 scoped correctness/topology/benchmark cell，禁止用旧结果覆盖新代码。
  4. 回填 SM86 环境、逐组合决策、原始产物位置与未覆盖硬件范围，更新 implementation report/debrief；把“候选路径可合入”与“生产默认 fused”拆成两个明确决策。
  5. 用户确认白名单策略后再实施：默认建议仍保持 split；若选择 fused，只允许证据支持的 TP1 decode QKV，并先补充与其资格范围匹配的硬件/工具链约束或额外跨架构数据。
- **Risks / open questions**:
  - 原始 suite 目录与新 LoRA fixture 目前不在工作区；没有这两项，无法把 PR 描述成自包含、可复核的正确性证据。
  - SM86 单架构结果不能证明 sm_90/sm_120 或不同 cuBLAS 选择下的数值与性能；这是是否修改 `Auto` 的核心决策，不是文档措辞问题。
  - 继续在当前 PR 叠加 split+RoPE 等新 kernel fusion 会使既有 64-run 数据失效；应先收敛本 issue，再将下一层融合按独立 A/B 处理。
...
- `pegainfer-qwen3/src/lib.rs`
  ...
  - 传入 launch/executor。
- `pegainfer-qwen3/src/projection_fusion.rs`（新增）
...
- `pegainfer-kernels/csrc/shared/fused_proj.cu`
- `pegainfer-kernels/src/ffi/shared.rs`
- `pegainfer-kernels/src/ops/elementwise.rs`，或一个更准确的 QKV split model-local wrapper位置
...
- `pegainfer-qwen3/src/batch_decode_dag.rs`
...
- `pegainfer-qwen3/src/batch_decode_trace.rs` 及 call-spec 相关文件
...
- `pegainfer-qwen3/kernel_manifests/qwen3.toml`
- `pegainfer-qwen3/src/bin/qwen3_kernel_report.rs` 及其 report 数据结构
...
PEGAINFER_TEST_MODEL_PATH=models/Qwen3-4B \
  cargo test --release -p pegainfer-qwen3 --test hf_golden_gate -- --nocapture
PEGAINFER_TEST_MODEL_PATH=models/Qwen3-4B \
  cargo test --release -p pegainfer-qwen3 --test lora_golden_gate -- --nocapture
```
...
- 对未启用组合的明确否决原因。
- 更新后的 `docs/models/qwen3/accuracy-gate.md`、相关 kernel/perf 文档、`pegainfer-kernels/KERNELS.md` 和本任务 Debrief。
...
- 新增 `pegainfer-qwen3/src/projection_fusion.rs`：
...
- 在 `pegainfer-kernels/csrc/shared/fused_proj.cu` 恢复纯 BF16 load/store 的
...
- HF golden 与 LoRA golden 新增环境选择：
  `PEGAINFER_QWEN3_PROJECTION_FUSION=split|qkv|gate-up|both`。每个模式通过
...
- GPU 主机首次执行暴露出 `cargo test --release --workspace --lib` 会编译
  GLM5.2/Kimi-K2，并通过 `pegainfer-kernels/moe` 拉入 DeepEP shim，导致
  Qwen3 专项验证额外要求 `PEGAINFER_NCCL_ROOT` 指向 NCCL ≥ 2.30.4。
- 将该门禁改为单个 Cargo invocation，只选择 `pegainfer-kernels`、
  `pegainfer-qwen3`、`pegainfer-server` 三个 Qwen3 直接相关 package；
...
- AutoDL 首次执行 scoped unit gate 后，`pegainfer-qwen3` 在 prefill 与
  unified forward 的 `ops::split_qkv_into` 调用处报 `E0425`；后续
  `qwen3_model_report` topology binary 也使用同一路径，尚未执行到。
- 根因是 operator 已从 `pegainfer-kernels::ops` 导出，但遗漏了
  `pegainfer-core::ops` facade re-export；Qwen3 其他常规 GPU operator
  均通过该 facade 使用。
...
### Step 15 — SM86 结果对齐与实际 topology 元数据

- 复核 Issue #746 全部评论：双卡 SM86/CUDA 12.6 suite 已完成，维护者已要求
  开 PR。按 suite 代码中的硬规则，八个独立组合只有 TP1 decode QKV 达到
  `ENABLE`，其余均为 `KEEP_SPLIT`。
...
- Rust release unit test 在 macOS 仍被 Linux-only `rdma-mummy-sys` headers 与无
  ...
  缺 `k_proj/gate_proj/up_proj`。
- 评估后不做 scratch enum 重排：当前 Option 只由构造器赋值，没有发现非法状态
  写入点；该改动不改善性能/正确性，却会扩大需要 Linux 重验的代码面。
...
- **Pitfalls encountered**:
  ...
  - 新 kernel operator 只在 kernels crate 导出不足以覆盖使用
    `pegainfer_core::ops` facade 的模型路径；lib 与 report binary 都必须在
    Linux 编译面验证。
