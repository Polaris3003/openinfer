...
| `models/qwen3/fused-projection-parity-plan.md` | Issue #746: current-head SM89 schema-v3 evidence is complete and numerically green, but all eight pre-registered independent performance decisions are `KEEP_SPLIT`; TP1 decode both is a follow-up signal, the tracked five-target fixture and hardware-scoped Auto policy remain open. |
| `models/qwen3/fused-projection-parity-implementation-report.md` | Issue #746 implementation report: fused candidates, fail-closed controls, LoRA/TP/graph/HTTP validation and current-head SM89 evidence are complete; no independent row qualifies for Auto, and the tracked five-target fixture remains the PR self-containment gap. |
| `models/qwen3/fused-projection-release-runbook.md` | Issue #746 release record: `a8ef928` passed 104/104 commands on 2×RTX 4090; all independent rows stay split, while TP1 decode both (`+2.35%` TPOT) is retained only as a pre-registered follow-up candidate. |
...
| Path | TL;DR |
| --- | --- |
| `subsystems/frontend/frontend-architecture.md` | `pegainfer-frontend` owns everything north of the model schedulers: engine contract (ex `pegainfer-engine`, now the `engine/` module tree), `vllm` protocol stack (ex `pegainfer-vllm-frontend`), `ModelLine` dispatch. All six model lines implement `ModelLine` in their own crates; `pegainfer-server` is a pure-dispatch bin. Next: prototype a `dynamo` in-process stack. |
