...
| `models/glm52/whole-step-decode-graph.md` | PR5c execution record: whole-step CUDA graph (one launch/rank/step) + weight-only fp8 GEMV for all m=1 projections + grid-strided device-bounded MoE quant chain + gate|up packed GEMV + shared-expert∥collectives overlap = **200 → 23.4 ms/step** on jz-38, 8-way free. Remaining ~1.4 ms to vLLM = collective latency floor + GEMV BW (#542); indexer oracle reference drift (#541). |
