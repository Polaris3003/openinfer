...
| `models/glm52/serving-status.md` | Living status: decode serving feature-complete for its scope. GLM5.2 P/D contributes to the H2 design-first effort; implementation remains Later. The model-specific design must cover target-tail transfer and DSpark suffix cold-start versus draft-KV transfer. |
| `models/glm52/tp4-gb300-bringup.md` | GLM5.2 TP4 on 4xGB300: FlashInfer sparse MLA, compact graphs, SM-selected GEMV/MoE launches, vocabulary-sharded greedy tail. 07-10 matched pair: ~parity with vLLM TP4 at 1/256 (after dropping the non-production short-context tier), ahead at 1024/256 (9.56 vs 9.60ms); a 07-11 re-validation reads a flat +0.54ms/step on the OpenInfer path only — host drift, not code. Functional gates green; re-baseline + TP4 golden open. |
