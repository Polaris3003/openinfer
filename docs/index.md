...
| `models/glm52/tp4-prefill-only.md` | PR1 establishes the TP4 prefill-only contract: prefix cache required, one active request, `max_tokens=1`, page-aligned 16K future native-prefill chunks, and VRAM-derived `max_model_len` after scratch reservation; large-M eager kernels remain follow-up. |
