...
| `models/qwen3/fused-projection-parity-plan.md` | Issue #746: fused candidates and validation are implemented; SM86 TP1/TP2 correctness is green and only TP1 decode QKV meets the performance rule, but raw evidence, the committed five-target LoRA fixture, and a hardware-scoped Auto policy remain open, so the default stays split. |
| `models/qwen3/fused-projection-parity-implementation-report.md` | Issue #746 implementation report: SM86 validation is complete and only TP1 decode QKV meets the rule; resolved topology/reasons are now benchmarked explicitly, while raw artifacts, the committed five-target LoRA fixture, and cross-architecture Auto qualification remain open. |
