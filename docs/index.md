...
| `models/qwen3/fused-projection-parity-plan.md` | Issue #746: QKV/gate-up candidates and fail-closed validation automation are implemented, including real-weight TP-rank numerical/algorithm reports and a 64-run E2E matrix summarizer; Linux CUDA execution and five-target LoRA fixture regeneration remain pending, so Auto stays split. |
| `models/qwen3/fused-projection-parity-implementation-report.md` | Detailed Issue #746 implementation report: traces CLI→resolved policy→buffers→decode/prefill/unified/verify, explains TP/LoRA/Graph invariants and memory costs, and separates implemented candidates from the still-missing Linux CUDA correctness/performance evidence. |
